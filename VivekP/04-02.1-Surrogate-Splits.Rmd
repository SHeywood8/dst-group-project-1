...
# Introduction to Surrogate Splits
...
> Definitions from Oxford Languages: a substitute [...]. (Synonyms included: proxy, replacement, stand-in.)
...
X_train <- read.csv("X_train.csv", row.names = 1)  # Set the first column as row names
y_train <- read.csv("y_train.csv", row.names = 1)  # Set the first column as row names
X_test <- read.csv("X_test.csv", row.names = 1)    # Set the first column as row names
y_test <- read.csv("y_test.csv", row.names = 1)    # Set the first column as row names
...
Since `education` and `education_num` encode the same information, we drop the `education` column. (In fact, it did not make a big difference to the tree whether we include it or not -- see the appendix.)
...
```{r}
# Load necessary libraries
library(caret)    # For precision and recall calculations
