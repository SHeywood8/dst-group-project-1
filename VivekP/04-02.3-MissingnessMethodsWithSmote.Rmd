...
We will explore other methods of dealing with missingness, and compare them.
...
**NOTE**: We preferred to do this rather than saving the files and loading them as it sometimes led to issues for the other group members.
...
We delete all missing data. This is not really a good strategy here because we have about 50000 observations and 3000 missing some values. This was not considered a negligible amount of missingness. We consider this as a baseline.
...
X_test <- na.omit(X_test)  # Ensure test set is free of NAs; this may remove some corresponding y_test rows
...
The results shocked us. The tree was quite robust to the method we chose, and the ROC curves and AUC were almost identical. Note that we have not considered imputation here, as we saw that the performance was not getting better regardless of the method we used, so we post-poned the use of imputation to when we consider ensembles.
